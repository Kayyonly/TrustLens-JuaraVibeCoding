...
import { exportInvestigationPdf } from "@/lib/exportInvestigationPdf"
  ...
  const handleExportPdf = async () => {
    try {
      ...
      await exportInvestigationPdf(result, {
        title: "TrustLens",
        subtitle: language === "id" ? "Laporan Investigasi TrustLens" : "TrustLens Investigation Report",
        generatedAt: language === "id" ? "Waktu laporan" : "Report time",
        analysisId: language === "id" ? "ID laporan" : "Report ID",
        sectionThreatSummary: language === "id" ? "Ringkasan Risiko" : "Risk Summary",
        sectionAiAnalysis: language === "id" ? "Temuan Analisis" : "Analysis Findings",
        sectionMetrics: language === "id" ? "Metrik Perilaku" : "Behavioral Metrics",
        sectionHeatmap: language === "id" ? "Bukti yang Disorot" : "Highlighted Evidence",
        sectionRecommendations: language === "id" ? "Langkah Aman" : "Safe Next Steps",
        sectionFooter: language === "id" ? "Disusun oleh TrustLens" : "Prepared by TrustLens",
        threatScore: t.scamProb,
        threatLevel: t.threatComplete,
        aiConfidence: t.aiConfidence,
        detectedCategory: t.categoryDetected,
        manipulation: t.manipulation,
        phishing: t.phishing,
        emotional: t.psychologyAnalysis,
        classification: t.threatComplete,
        urgencyManipulation: t.urgencyManipulation,
        trustExploitation: t.trustExploitation,
        isolationTactics: t.isolationTactics,
      })
      setExportMessage({ type: "success", text: language === "id" ? "Laporan investigasi berhasil dibuat." : "Investigation report exported." })
    } catch (error) {
      console.error("PDF export failed:", error)
      setExportMessage({ type: "error", text: language === "id" ? "PDF belum bisa dibuat. Coba lagi." : "The PDF could not be generated. Please try again." })
    } finally {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8 md:space-y-10 mt-14 w-full">
      ...
      <motion.div variants={itemVariants} className="px-2">
        <div className="w-full rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-[0_0_40px_rgba(255,255,255,0.02)]">
          <p className="text-white/60 text-sm">{language === "id" ? "Simpan temuan sebagai laporan investigasi." : "Save these findings as an investigation report."}</p>
          <button
            onClick={handleExportPdf}
            disabled={isExporting}
            className="group h-11 px-5 rounded-2xl border border-white/15 bg-black/40 text-white text-sm font-medium hover:bg-white/10 hover:border-white/30 transition-all duration-300 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {isExporting ? (language === "id" ? "Menyusun PDF..." : "Preparing PDF...") : (language === "id" ? "Export Laporan" : "Export Report")}
          </button>
        </div>
        {exportMessage && (
          <div className={`mt-3 rounded-2xl px-4 py-3 text-sm border ${exportMessage.type === "success" ? "text-white/85 border-white/20 bg-white/10" : "text-red-200 border-red-300/20 bg-red-950/30"}`}>
            {exportMessage.text}
          </div>
        )}
      </motion.div>
