  ...
  const handleExportPdf = async () => {
    try {
      ...
      await exportInvestigationPdf(result, {
        title: "TrustLens",
        subtitle: language === "id" ? "Laporan Investigasi AI" : "AI Investigation Report",
        generatedAt: language === "id" ? "Dibuat pada" : "Generated at",
        analysisId: language === "id" ? "ID Analisis" : "Analysis ID",
        sectionThreatSummary: language === "id" ? "Ringkasan Ancaman" : "Threat Summary",
        sectionAiAnalysis: language === "id" ? "Analisis AI" : "AI Analysis",
        sectionMetrics: language === "id" ? "Metrik Ancaman" : "Threat Metrics",
        sectionHeatmap: language === "id" ? "Ringkasan Heatmap" : "Threat Heatmap Summary",
        sectionRecommendations: language === "id" ? "Rekomendasi" : "Recommendations",
        sectionFooter: language === "id" ? "Dihasilkan oleh TrustLens AI" : "Generated by TrustLens AI",
      ...
      setExportMessage({ type: "success", text: language === "id" ? "Laporan AI berhasil diunduh." : "AI report exported successfully." })
    } catch (error) {
      console.error("PDF export failed:", error)
      setExportMessage({ type: "error", text: language === "id" ? "Gagal mengekspor PDF. Coba lagi." : "Failed to export PDF. Please try again." })
    } finally {
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8 md:space-y-10 mt-14 w-full">
      ...
      <motion.div variants={itemVariants} className="px-2">
        <div className="w-full rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-[0_0_40px_rgba(255,255,255,0.02)]">
          <p className="text-white/60 text-sm">{language === "id" ? "Ekspor laporan investigasi AI premium." : "Export a premium AI investigation dossier."}</p>
          <button
          ...
          >
            {isExporting ? (language === "id" ? "Membuat PDF..." : "Generating PDF...") : (language === "id" ? "Export Laporan AI" : "Export AI Report")}
          </button>
        </div>
        {exportMessage && (
